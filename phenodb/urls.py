...
urlpatterns = patterns('search.views',
    ...
    url(r'^search/querybuilder/(?P<page>\d+)/(?P<results_per_page>\d+)/(?P<tables_str>\w+)/(?P<where_str>\w+)/(?P<whereis_str>\w+)/(?P<querystr_str>[\w\.]*)/$', 'querypage', name="search-querypage"),
    url(r'^search/querybuilder/(?P<tables_str>\w+)/(?P<where_str>\w+)/(?P<whereis_str>\w+)/(?P<querystr_str>[\w\.]*)/$', 'query_export', name="search-exportcsv"),
